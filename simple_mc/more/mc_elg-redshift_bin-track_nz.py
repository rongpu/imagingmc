# Compare the N(z) in the three imaging regions
# salloc -N 1 -C cpu -q interactive -t 04:00:00
# python mc_elg-redshift_bin-track_nz.py --zmin 0. --zmax 1.7 --region bassmzls
...
parser.add_argument('--region', required=False, default='All')
...
hpx_output_dir = '/global/cfs/cdirs/desi/users/rongpu/imaging_mc/mc/20240513-track_nz'
hpx_output_fn = 'mc_elg_desi_ebv_{}_{}'.format(zmin, zmax)
tmp_dir = '/pscratch/sd/r/rongpu/imaging_mc/tmp6/'
...
for jj, randoms_path in enumerate(randoms_paths):
    ...
    output_path = os.path.join(tmp_dir, 'mc_elg_'+os.path.basename(randoms_path).replace('-trim.fits', '-{}_{}-elgmask_v1-{}.fits'.format(zmin, zmax, region)))
    ...
    if region.lower()=='all':
    ...
    elif region.lower()=='bassmzls':
        mask = cat['PHOTSYS']=='N'
    elif region.lower()=='decals':
        des_footprint = Table.read(os.path.expanduser('/global/cfs/cdirs/desi/users/rongpu/useful/in_des/hp_in_des_{}_ring.fits.gz'.format(256)))
        hpix = hp.ang2pix(256, cat['RA'], cat['DEC'], nest=False, lonlat=True)
        isdes = np.array(des_footprint['in_des'][hpix])
        mask = cat['PHOTSYS']=='S'
        mask &= ~isdes
    elif region.lower()=='des':
        des_footprint = Table.read(os.path.expanduser('/global/cfs/cdirs/desi/users/rongpu/useful/in_des/hp_in_des_{}_ring.fits.gz'.format(256)))
        hpix = hp.ang2pix(256, cat['RA'], cat['DEC'], nest=False, lonlat=True)
        isdes = np.array(des_footprint['in_des'][hpix])
        mask = cat['PHOTSYS']=='S'
        ...
        raise ValueError
    cat = cat[mask]
    print('Cut to region {}%'.format(len(cat)))
...
if not os.path.isdir(hpx_output_dir):
    os.makedirs(hpx_output_dir)
...
nz_path = os.path.join(hpx_output_dir, 'mc_elg_'+os.path.basename(randoms_path).replace('-trim.fits', '-{}_{}-elgmask_v1-nz-{}.fits'.format(zmin, zmax, region)))
if track_nz:
    nz = Table()
    nz['zmin'], nz['zmax'] = nz_bins[:-1], nz_bins[1:]
    nz['count'] = nz_sum
    nz.write(nz_path, overwrite=True)
