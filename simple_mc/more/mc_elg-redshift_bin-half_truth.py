...
# python mc_elg-redshift_bin-half_truth.py --zmin 0.8 --zmax 1.1 --half 1
# python mc_elg-redshift_bin-half_truth.py --zmin 0.8 --zmax 1.1 --half 2
# python mc_elg-redshift_bin-half_truth.py --zmin 1.1 --zmax 1.6 --half 1
# python mc_elg-redshift_bin-half_truth.py --zmin 1.1 --zmax 1.6 --half 2
...
parser.add_argument('--half', required=True, help='which half, 1 or 2?')
...
hpx_output_dir = '/global/cfs/cdirs/desi/users/rongpu/imaging_mc/mc/20240513-redshift_bin-half_truth'
hpx_output_fn = 'mc_elg_desi_ebv_{}_{}-half_{}'.format(zmin, zmax, args.half)
tmp_dir = '/pscratch/sd/r/rongpu/tmp/imaging_mc/tmp1/'
...
for jj, randoms_path in enumerate(randoms_paths):
    ...
    output_path = os.path.join(tmp_dir, 'mc_elg_'+os.path.basename(randoms_path).replace('-trim.fits', '-{}_{}-elgmask_v1-half_{}.fits'.format(zmin, zmax, args.half)))
...
tcount_path = os.path.join(hpx_output_dir, 'mc_elg_'+os.path.basename(randoms_path).replace('-trim.fits', '-{}_{}-elgmask_v1-tcount-half_{}.fits'.format(zmin, zmax, args.half)))
if count_truth:
    tcount.write(tcount_path, overwrite=overwrite)

nz_path = os.path.join(hpx_output_dir, 'mc_elg_'+os.path.basename(randoms_path).replace('-trim.fits', '-{}_{}-elgmask_v1-nz-half_{}.fits'.format(zmin, zmax, args.half)))
if track_nz:
    nz = Table()
