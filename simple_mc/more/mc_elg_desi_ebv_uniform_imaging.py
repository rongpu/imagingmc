...
# Only EBV variation; Make the depth and seeing uniform across the footprint
# WORK IN PROGRESS
...
tmp_dir = '/pscratch/sd/r/rongpu/imaging_mc/tmp10/'
...
for randoms_path in randoms_paths:
    ...
    # Assign uniform imaging properties
    if randoms_path==randoms_paths[0]:
        imsys_dict = {}
        for col in ['psfsize_g', 'psfsize_r', 'psfsize_z', 'psfdepth_g', 'psfdepth_r', 'psfdepth_z']:
            imsys_dict[col] = np.median(cat[col])

    for col in ['psfsize_g', 'psfsize_r', 'psfsize_z', 'psfdepth_g', 'psfdepth_r', 'psfdepth_z']:
        cat[col] = imsys_dict[col]
...
hp_table.write('/global/cfs/cdirs/desi/users/rongpu/imaging_mc/mc/20230201/mc_elg_randoms_elgmask_v1_desi_ebv_uniform_imaging_healpix_{}.fits'.format(nside), overwrite=True)
